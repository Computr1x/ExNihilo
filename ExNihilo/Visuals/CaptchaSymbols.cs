	...
	public override void Render(Image image, GraphicsOptions graphicsOptions)
	{
		...
		image.Mutate(x =>
		{
			...
			foreach (var symbolsParams in TextSymbols.RandomizedTextProperties)
			{
				...
				tempImg.Mutate(y =>
				{
                    var visualType = (VisualType) symbolsParams.Type;

                    y.DrawText(
                        dopt,
                        opt,
                        symbolsParams.Content,
                        visualType.HasFlag(VisualType.Filled) ? symbolsParams.Brush.Value : null,
                        visualType.HasFlag(VisualType.Outlined) ? symbolsParams.Pen.Value : null
                    );
				});
